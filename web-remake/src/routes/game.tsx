        ...
        <div className="relative z-10 max-w-2xl w-full text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button onClick={() => setMode({ time: null, lives: 3, name: 'ZEN' })} className="group relative p-6 border border-cyan-500/30 bg-cyan-950/20 hover:bg-cyan-900/40 hover:border-cyan-400 rounded-xl transition-all text-left overflow-hidden">
              <div className="text-cyan-400 font-bold text-xl mb-1 group-hover:scale-105 transition-transform">🧠 Modo ZEN</div>
            ...
            <button onClick={() => setMode({ time: 30, lives: 3, name: 'NORMAL' })} className="group relative p-6 border border-green-500/30 bg-green-950/20 hover:bg-green-900/40 hover:border-green-400 rounded-xl transition-all text-left">
              <div className="text-green-400 font-bold text-xl mb-1 group-hover:scale-105 transition-transform">⏱️ Modo NORMAL</div>
            ...
            <button onClick={() => setMode({ time: 10, lives: 3, name: 'HARD' })} className="group relative p-6 border border-orange-500/30 bg-orange-950/20 hover:bg-orange-900/40 hover:border-orange-400 rounded-xl transition-all text-left">
              <div className="text-orange-400 font-bold text-xl mb-1 group-hover:scale-105 transition-transform">🔥 Modo HARD</div>
            ...
            <button onClick={() => setMode({ time: 5, lives: 2, name: 'NIGHTMARE' })} className="group relative p-6 border border-red-500/30 bg-red-950/20 hover:bg-red-900/40 hover:border-red-500 rounded-xl transition-all text-left">
              ...
              <div className="text-red-500 font-bold text-xl mb-1 group-hover:scale-105 transition-transform relative z-10">💀 NIGHTMARE</div>
  ...
  return (
    <div className="game-container bg-[#08071a] text-slate-200 min-h-screen flex flex-col font-sans w-full max-w-[100vw] overflow-x-hidden">
      ...
      <header className="flex flex-wrap gap-4 items-center justify-between p-4 border-b border-cyan-500/10 bg-[#050414] shadow-md z-20 relative">
        ...
        <div className="flex gap-8 items-center bg-white/5 px-6 py-2 rounded-full border border-cyan-500/20">
      ...
      <main className="flex flex-col md:flex-row flex-1 overflow-hidden relative">
        <div id="map-wrap" className="relative flex-1 flex items-center justify-center p-4 bg-[#0a081a] overflow-auto shadow-[inset_0_0_100px_rgba(0,0,0,0.8)]">
          <canvas 
            ref={canvasRef}
            id="map" 
            width="880" 
            height="510" 
            className="block rounded-xl border border-cyan-500/20 w-full max-w-[880px] h-auto aspect-[880/510] shadow-[0_0_40px_rgba(0,245,255,0.05)] cursor-crosshair touch-none"
          ></canvas>
        </div>
        
        <aside className="w-full md:w-[320px] flex-none p-6 flex flex-col gap-6 border-l border-cyan-500/10 bg-[#0d0a24] overflow-y-auto relative z-10 shadow-2xl">
      ...
      <div id="modal" className="hidden fixed inset-0 bg-black/80 backdrop-blur-sm z-50 items-center justify-center p-4 transition-all duration-300">
        <div className="bg-[#0f0e2a] border border-cyan-500/30 rounded-2xl p-8 sm:p-10 max-w-[480px] w-full text-center shadow-[0_0_80px_rgba(0,245,255,0.15)] relative overflow-hidden">
          ...
          <button id="modal-close-btn" className="mt-6 px-8 py-2 border border-slate-700 text-slate-400 rounded-full hover:bg-slate-800 hover:text-white transition-colors hidden mx-auto" onClick={() => (window as any).closeModal()}>Fechar</button>
        </div>
      </div>

      <div id="win" className="hidden fixed inset-0 bg-[#08071a]/95 backdrop-blur-md z-[60] items-center justify-center flex-col text-center p-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
           <button className="px-10 py-4 rounded-full bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-bold hover:scale-105 transition-transform shadow-lg shadow-cyan-500/20" onClick={() => (window as any).resetGame()}>Tentar Novamente</button>
           <button className="px-10 py-4 rounded-full border-2 border-slate-700 text-slate-300 font-bold hover:bg-slate-800 transition-colors" onClick={() => window.location.reload()}>Trocar Modo</button>
        </div>
      </div>
    </div>
  )
